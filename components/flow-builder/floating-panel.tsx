...
import type { FlowNode, DataSource } from "@/types/flow"
import {
  X,
  ArrowRight,
  Trash2,
...
export function FloatingPanel({ node, position, onClose, onUpdate, onDelete }: FloatingPanelProps) {
  ...
  const handleAddDataSource = (dataSource: DataSource) => {
    const currentSources = node.simulatedSources || []
    const newSources = [...currentSources, dataSource]
    onUpdate(node.id, { simulatedSources: newSources })
  }
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      ...
      // Don't close if clicking on the data source modal or its backdrop
      if (showDataSourceModal) {
        return
      }

      // Don't close if clicking on floating panel or flow node
    ...
    <>
      ...
      >
        ...
        <div className="space-y-3">
          ...
          {/* Data Sources */}
          <div>
            <label className="block text-xs font-medium text-gray-700 mb-1">Data Sources</label>

            {/* Show existing data sources */}
            {node.simulatedSources && node.simulatedSources.length > 0 && (
              <div className="space-y-1 mb-2">
                {node.simulatedSources.map((source, index) => (
                  <div
                    key={index}
                    ...
                    <div className="flex items-center gap-2 flex-1 min-w-0">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          <span className="truncate text-gray-700 font-medium">{source.label}</span>
                          <span className="text-gray-400 text-xs">({dataSourceTypeLabels[source.type]})</span>
                        </div>
                        <div className="flex items-center gap-3 text-xs text-gray-500 mt-0.5">
                          <span>{source.volume.toLocaleString()}/mo</span>
                          <span>{(source.errorRate * 100).toFixed(1)}% errors</span>
                        </div>
                      </div>
                    </div>
                    <button
            ...
            <button
              ...
              className="w-full px-2 py-1.5 border border-gray-200 rounded-md text-sm text-left text-gray-500 hover:bg-gray-50 focus:outline-none focus:ring-1 focus:ring-violet-500 focus:border-transparent transition-colors flex items-center gap-2"
            >
      ...
      {/* Data Source Modal - Render outside the floating panel to avoid z-index issues */}
      {showDataSourceModal && (
        <DataSourceModal
          isOpen={showDataSourceModal}
          onClose={() => setShowDataSourceModal(false)}
          onSubmit={handleAddDataSource}
          existingSources={node.simulatedSources}
        />
      )}
    </>
  )
