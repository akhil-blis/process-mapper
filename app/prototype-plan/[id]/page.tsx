...
const initialPrototypePlan: PrototypePlan = {
  screens: [
      ...
      elements: [
        {
          id: "elem-1",
          type: "input",
          label: "Drag and drop resumes or upload files",
          screenId: "screen-1",
          position: { x: 0, y: 0 },
        },
        {
    ...
    },
    {
      ...
      elements: [
        ...
        },
        {
          id: "elem-4",
          type: "action",
          label: "Match Against Role Criteria",
          screenId: "screen-2",
          position: { x: 4, y: 0 },
        },
      ],
    ...
    {
      id: "screen-3",
      title: "View Candidate Scores",
      position: { x: 4, y: 0 },
      elements: [
        {
          id: "elem-5",
          type: "info",
          label: "Compatibility score for each candidate",
          screenId: "screen-3",
          position: { x: 0, y: 0 },
        },
        {
          id: "elem-6",
          type: "info",
          label: "Reasoning summary for each score",
          screenId: "screen-3",
          position: { x: 1, y: 0 },
        },
        {
    ...
    {
      id: "screen-4",
      title: "Review Ranked Recommendations",
      position: { x: 6, y: 0 },
      elements: [
        {
          id: "elem-8",
          type: "info",
          label: "Top ranked candidates flagged for review",
          screenId: "screen-4",
          position: { x: 0, y: 0 },
        },
        {
          id: "elem-9",
          type: "action",
          label: "Approve or Reject Candidates",
          screenId: "screen-4",
          position: { x: 1, y: 0 },
        },
      ],
  ...
  connections: [
    {
      id: "conn-1",
      fromElementId: "elem-2",
      toScreenId: "screen-2",
    },
    {
      id: "conn-2",
      fromElementId: "elem-4",
      toScreenId: "screen-3",
    },
    {
      id: "conn-3",
      fromElementId: "elem-7",
      toScreenId: "screen-4",
    },
  ],
}
...
export default function PrototypePlanPage() {
  ...
  const handleAISuggestionForPrototype = (suggestionText: string) => {
    if (suggestionText === "add_candidate_feedback") {
      // Check if the candidate feedback screen already exists
      const existingFeedback = prototypePlan.screens.find((screen) => screen.title === "Candidate Feedback")

      if (!existingFeedback) {
        // Create the new Candidate Feedback screen
        const newScreen: PrototypeScreen = {
          id: `screen-${Date.now()}`,
          title: "Candidate Feedback",
          position: { x: 8, y: 0 }, // Position it after the last screen
          elements: [
            {
              id: `element-${Date.now()}-1`,
              type: "input",
              label: "Collect Interview Feedback",
        ...
        // Find the "Approve or Reject Candidates" element to connect from
        const reviewScreen = prototypePlan.screens.find((screen) => screen.title === "Review Ranked Recommendations")
        const approveRejectElement = reviewScreen?.elements.find(
          (element) => element.label === "Approve or Reject Candidates",
        )

        // Create connection from "Approve or Reject Candidates" to new feedback screen
        const newConnection: PrototypeConnection = {
          id: `conn-${Date.now()}`,
          fromElementId: approveRejectElement?.id || "",
